...
class ChtcScheddCpuOspoolFormatter(BaseFormatter):
    ...
    def get_table_title(self, table_file, report_period, start_ts, end_ts):
        ...
        if report_period in ["daily"]:
            ...
            title_str = f"CHTC per {info['agg'].rstrip('s')} usage for jobs completed on <strong>{start_date}</strong> on the OSPool"
        elif report_period in ["weekly", "monthly"]:
            end = datetime.fromtimestamp(end_ts)
            start_date = start.strftime("%Y-%m-%d")
            end_date = end.strftime("%Y-%m-%d")
            title_str = f"CHTC per {info['agg'].rstrip('s')} usage for jobs completed from <strong>{start_date} to {end_date}</strong> on the OSPool"
        else:
            end = datetime.fromtimestamp(end_ts)
    ...
    def get_subject(self, report_period, start_ts, end_ts, **kwargs):
        ...
        if report_period in ["daily", "weekly", "monthly"]:
            ...
            start_date = start.strftime("%Y-%m-%d %H:%M:%S")
            end_date = end.strftime("%Y-%m-%d %H:%M:%S")
            subject_str = f"CHTC AP on OSPool Usage Report {start_date} to {end_date}"
        return subject_str
